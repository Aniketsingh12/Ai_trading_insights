...
/**
 * The atmosphere plate behind the whole app.
 *
 * Two passes of the same clip through different colour grades — the second
 * lifted and blended over the first — then a heavy scrim. The scrim is doing
 * real work here rather than decoration: this app is dense with tables and
 * numbers, so the plate has to read as texture at the edges and vanish
 * completely under content.
 *
 * Fixed and pointer-events:none, so it never participates in scrolling or
 * hit-testing.
 */
export default function VideoStage() {
  ...
  useEffect(() => {
    ...
    // Autoplay can still be refused (data saver, some mobile browsers). Nothing
    // depends on the video, so a rejection is ignored rather than surfaced.
    vids.forEach((v) => v.play?.().catch(() => {}));

    // The two passes are only convincing while they're the same frame, and
    // independent <video> elements drift. Nudge the followers back to the
    // master when they slip past a threshold a viewer could notice.
    const [master, ...followers] = vids;
    const sync = () => {
      followers.forEach((v) => {
  ...
  return (
    <>
      <div className="stage" aria-hidden="true">
        <div className="stage-bg">{video(0)}</div>
        <div className="stage-bg2">{video(1)}</div>
